...
FORK_ID="$fork_child_id" python3 -c 'import json,sys,os; d=json.load(sys.stdin); assert d["passportVersion"] == "noeone.actor-passport.v5"; assert d["actor"]["id"] == os.environ["FORK_ID"]; assert d["continuity"]["ancestry"] is not None; assert d["recognition"]["assessmentCount"] == 0; assert d["recognition"]["verification"]["verified"] is True; assert d["institutional"]["commitmentCount"] == 0; assert d["institutional"]["evidenceArtifactCount"] == 0; assert d["authority"]["grantCount"] == 0; assert d["authority"]["effectiveActiveCount"] == 0' <<<"$fork_passport"
...
ALPHA_ID="$alpha_id" python3 -c 'import json,sys,os; d=json.load(sys.stdin); assert d["passportVersion"] == "noeone.actor-passport.v5"; assert d["actor"]["id"] == os.environ["ALPHA_ID"]; assert d["verification"]["valid"] is True; assert d["verification"]["continuity"]["valid"] is True; assert d["recognition"]["assessmentCount"] == 0; assert d["recognition"]["verification"]["verified"] is True; assert d["career"]["canonicalEvents"] >= 9; assert d["continuity"]["transitionCount"] >= 1; assert d["institutional"]["verification"]["valid"] is True; assert d["institutional"]["evidenceArtifactCount"] == 1; assert d["institutional"]["evidenceBindingCount"] == 1; assert d["institutional"]["evidenceValidationCount"] == 1; assert d["institutional"]["evidenceValidationsByStatus"]["verified"] == 1; assert d["institutional"]["commitmentCount"] == 1; assert d["institutional"]["commitmentsByStatus"]["fulfilled"] == 1; assert d["authority"]["grantCount"] == 1; assert d["authority"]["grantsByStatus"]["revoked"] == 1; assert d["authority"]["effectiveActiveCount"] == 0; assert d["authority"]["verification"]["valid"] is True' <<<"$passport_json"
...
echo "NOEONE frozen install, authentication, ownership isolation, delegated authority, scope attenuation, cascade revocation, governed continuity, contextual recognition, normalized evidence, institutional commitments, fork non-inheritance, match execution, canonical verification, actor passport v5, and production web smoke tests passed."